...
struct Instrucoes analisar_instrucao(char instrucao[]) {
    ...
    struct Instrucoes analisada;

    char *token = strtok(instrucao, " ,");
    strcpy(analisada.opcode, token);

    if (strcmp(analisada.opcode, "add") == 0 || strcmp(analisada.opcode, "sll") == 0 || strcmp(analisada.opcode, "or") == 0) {
        // Formato R
        token = strtok(NULL, " ,");
        strcpy(analisada.rd, token);

        token = strtok(NULL, " ,");
        strcpy(analisada.rs1, token);

        token = strtok(NULL, " ,");
        strcpy(analisada.rs2, token);

        strcpy(analisada.imm, "");

    } else if (strcmp(analisada.opcode, "andi") == 0 || strcmp(analisada.opcode, "addi") == 0 || strcmp(analisada.opcode, "mv") == 0) {
        // Formato I
        token = strtok(NULL, " ,");
        strcpy(analisada.rd, token);
        ...
        token = strtok(NULL, " ,");
        strcpy(analisada.imm, token);

        strcpy(analisada.rs2, "");
        
    } else if (strcmp(analisada.opcode, "lui") == 0) {
        // Formato U
        token = strtok(NULL, " ,");
        strcpy(analisada.rd, token);
